...
function Victory takes nothing returns nothing
	...
	call DisplayTextToForce(bj_FORCE_ALL_PLAYERS,("|CFFFF00B2决战江湖1.67的游戏总评分："+(I2S(ae)+"分（通关）")))
...
function Lose takes nothing returns nothing
	local integer i=0
	call DisplayTextToForce(bj_FORCE_ALL_PLAYERS,("|CFFFF00B2决战江湖1.67的游戏总评分："+(I2S(ae)+"分（战败）")))
	set i = 1
	loop
		exitwhen i >= 6
